...
void initRestaurant(Restaurant* restaurant)
{
    ...
    Warehouse& ware = temp[DRINK_DEPARTMENT]->getWarehouse();
...
bool validateYesNoInput(const string& prompt) {
    char answer;
    while (true) {
        cout << prompt;
        cin >> answer;
...
bool gatherDrinkInfo(char* mealName, int& price, int& volume, int& glassType, bool& special)
{
    ...
    while (!validInput)
    {
        try 
        {
            ...
            if (mealName==nullptr || mealName[0]=='\0')
            {
				cout << "Drink name cannot be empty!\n";
				return false;
            }
            ...
            special = validateYesNoInput("Is this a special drink? y/n\n");
            cout << "Which glass do you want? Enter number:\n0: Lowball\n1: Wine\n2: Beer\n3: Highball\n4: Paper cup\n";
            cin >> glassType;
            if (glassType < 0 || glassType > 4) 
            {
                cout<<"Invalid glass type selected!";
                return false;
            }
...
bool gatherDrinkIngredients(Ingredient** ingredientList, int& numOfIngredients, Restaurant* restaurant) 
{
...
bool gatherFoodInfo(char* mealName, int& price, bool& special, bool& kosher)
{
    bool validInput = false;
    while (!validInput) 
    {
        try 
        {
	        ...
	        cin.getline(mealName, MAX_NAME_LENGTH + 1);
            if (mealName == nullptr || mealName[0] == '\0') {
                cout << "Meal name cannot be empty!\n";
                return false;
            }
            ...
            special = validateYesNoInput("Is this a special meal? y/n\n");
            kosher = validateYesNoInput("Is this a kosher meal? y/n\n");
...
bool gatherFoodIngredients(Ingredient** ingredientList, int& numOfIngredients, Restaurant* restaurant)
{
    char ingredientName[MAX_NAME_LENGTH + 1];
    int quantity;
    char answer = 'n';
    do 
    {
        ...
        while (!validInput) 
        {
            try 
            {
                ...
                Department** temp = restaurant->getDepartments();
                Warehouse& ware = temp[FOOD_DEPARTMENT]->getWarehouse();
                Ingredient* ingredientToCopy = ware.getIngredientByName(ingredientName);
                ingredientList[numOfIngredients] = new Ingredient(ingredientName, ingredientToCopy->getSection(), quantity);
                numOfIngredients++;
                validInput = true;
            ...
            try 
            {
                switch (choice) 
                {
		            ...
		            case 5: // Add meal food to menu
                    {
                        ...
                        if(gatherFoodInfo(mealName, price, special, kosher))
                            if(gatherFoodIngredients(ingredientList, numOfIngredients, restaurant))
